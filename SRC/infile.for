...
c     kfactor(1) - lower limit of conductivity for frozen soil in forest
c     kfactor(2) - lower limit of conductivity for frozen soil in pasure/pere
c     kfactor(3) - lower limit of conductivity for frozen soil in annual crops/fallow
c
      ksnowf = -1
      kresf = -1
      ksoilf = -1
...
310   if ((fineTop.gt.10).or.(fineTop.lt.1)) fineTop=10
      ...
      if ((kfactor(1).le.0.0).or.(kfactor(1).gt.1.0)) kfactor(1) = 0.5
      if ((kfactor(2).le.0.0).or.(kfactor(2).gt.1.0)) kfactor(2) = 1e-5
      if ((kfactor(3).le.0.0).or.(kfactor(3).gt.1.0)) kfactor(3) = 1e-5
      close(22)
      go to 302
 301  fineTop = 10
      fineBot = 10
...
c
 ...
 1300 format (//10x,'USDA WATER EROSION PREDICTION PROJECT',/,10x,
     1    37('-'),//,10x,'HILLSLOPE PROFILE WITH WQ (swat)',/,20x,
