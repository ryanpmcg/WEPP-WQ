...
c     modify: hmann
c
      include 'csolva1.inc'  !! DSHI added to pass OM info to SWAT
      include 'csolva2.inc'
c
      include 'cstore.inc'
...
c
     ...
     1    train,yirrig,yirrro,yrmro,yrrro,fuzzr,powerz,hyradz
       ...
       real wq_sedyldMT,wq_areaOFE
c
      character text2(12)*3
      character*8 inifil
c     DSHI
      character*51 wq_CropName
c
c     + + + LOCAL DEFINITIONS + + +
c
c     Integer Variables:
...
c         write(*,*)jyear,jyear,jyear,'year'
          else
            switch(iplane) = 0
          end if
   90   continue
...
c             write(*,*) runoff(1),runvol(1),'runoff'
...
c
c DSHI        chemical content and transport
c
              wq_sedyldMT = 0.0
cd              if (nowcrp(iplane) > 1 ) PAUSE

c              wq_CropName = crpnam(nowcrp(iplane))

cd             wq_CropName = crpnam(itype(nowcrp(iplane),iplane))
               wq_CropName = crpnam(itype(nowcrp(iplane),iplane))
c               write(*,*) wq_CropName



              do kk = 1, 10    !!DSHI hardwired cnpart
                !!DSHI added for SWAT -- in metric tons
                wq_sedyldMT = wq_sedyldMT + ggs(kk,1)* 16.020973/1000.0
cd                IF (ggs(kk,1) > 0.) PAUSE
              enddo
              wq_sedyldMT = avsole*fwidth(1)/1000

              wq_areaOFE = fwidth(1)*totlen(1)    !!DSHI this is whole watershed area
                                              !! assigned to OFE specifically for EAHM

              wq_drainq = drainq(iplane)
c              write(*,*) wq_sedyldMT,fwidth(1),'sediment yield\width'

              if (wq_runWQ.eq.1) then
                 call fromWeppToSwat()

                 write(*,*) "***** Chemical Subroutine Called *****"
                 call chemical(bd,st,wq_sol_prk,ul,
     &                    thetdr,dg,fc,solthk,nsl(1),1,sdate,
     &                    wq_fphu,wq_areaOFE,runoff(1),rmogt,smrm,
     &                    obmaxt,obmint,tmin,tmax,tave,radly,salb(1),
     &                    mon,day,wq_sedyldMT,iplane,rain(iplane),              !fin*1000.0,
     &                    soilw*1000.0,jyear,ncrop,wq_CropName,orgmat,
     &                    lai(iplane),jdharv(nowcrp(iplane),iplane),
     &                    jdplt(nowcrp(iplane),iplane),wq_drainq)                !!DSHI added FPHU for SWAT

c                write(*,*) drainq(iplane)*1000,wq_drainq,'iplane'
