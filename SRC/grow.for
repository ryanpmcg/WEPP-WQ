...
c       stress (temstr).
c
c       DSHI - added strsn, strsp - force temstr to 1?
        if (wq_runWQ.eq.1) then 
           temstr(iplane) =1.
           reg(iplane) = amin1(watstr(iplane),temstr(iplane),wq_strsn,
     1      wq_strsp)
c        write(*,*) watstr(iplane),temstr(iplane),wq_strsn,wq_strsp
        else
...
c
!!DSHI
      wq_dryBioMass = vdmt(iplane)*10000.0   ! DSHI added to carry to SWAT
      wq_rootMass = rtmass(iplane)*10000.0   ! DSHI added to carry to SWAT
      wq_HINDX = hia(iplane)
      wq_strsw = watstr(iplane)
      wq_strst = temstr(iplane)
      wq_fphu = fphu
c      write(*,*)wq_dryBioMass,wq_rootMass,vdmt(iplane),'sdate'
!!      if (strsn .le. 1 .OR. strsp .le. 1) write (*,*)                   &
cd      if (watstr(iplane) .lt. 1 .OR. temstr(iplane) .lt. 1) write (*,*) &
cd     &"         NUTRIENT STRESS ::==  N = ",STRSN,"\n   P = ",STRSP,    &
cd     &"\n   WATSTR = ",watstr(1),"\n   TEMSTR = ",temstr(1),            &
cd     &"\n   Bio Mass = ",dryBioMass
