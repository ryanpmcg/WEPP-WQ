...
c
c     check to see if we should use the water quality code
c      
      wq_runWQ = 0
      open(unit=22,file='wepp_wq.txt',status='old',err=301)
      wq_runWQ = 1
      write(6,4000)
      close(22)
      goto 302
...
c
 ...
 3400 format (' *** NOTE ***',/,' Y OR N FOR EXISTING PASS FILE',/,
     1    ' N assumed',/,' *** NOTE ***',/)
 4000 format (//'->WEPP Water Quality Option Set'//)
 4001 format (//'->WEPP Hourly water balance Set (UI code)'//)   
      end
